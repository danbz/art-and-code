...
//--------------------------------------------------------------
void ofApp::setup(){
    // load in our emoji font
    ofTrueTypeFontSettings settings("Apple Color Emoji.ttc", 20);
    ...
    cout << "length of array of chars = " << ofUTF8Length( asciiCharacters) << endl;
...
void ofApp::draw(){
    ...
    for (int i = 0; i < camWidth; i+= w){
        // inside this is another loop that loops from 0 to the camHeight in steps of 9
        for (int j = 0; j < camHeight; j+= h){
            // get the pixel and its lightness (lightness is the average of its RGB values)
            float lightness = pixelsRef.getColor(i,j).getLightness();
            
            // calculate the index of the character from our asciiCharacters array
            int character = powf( ofMap(lightness, 0, 255, 0, 1), 2.5) * ofUTF8Length(asciiCharacters);
            
            // draw the character at the correct location
            // font.drawString(ofToString(asciiCharacters[character]), i, j);
            font.drawString( ofUTF8Substring(asciiCharacters, character, 1), i, j);
            
            // cout << ofToString(asciiCharacters[character]) << endl;
        }
    }
    ...
    font.drawString(asciiCharacters, 20, 240);
    font.drawString("🍱 🍲 🍳 🍴 🍵 🍶 🍷 🍸 🍹 🍺", 20, 100);
}
